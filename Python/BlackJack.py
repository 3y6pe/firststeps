...
class Round:
    ...
    def _hit(self):
        self.player.hand.extend(self.deck.deal_card())
    ...
    def next_phase(self):
        for self.player in self.players[:-1]:
            self.print_table()
            if self.player.current_bet * 2 <= self.player.bankroll:
    ...
    def _win_check(self):
        ...
        if self.player.points == 21 and dealer.points != 21:
        ...
        elif 21 > self.player.points > dealer.points or dealer.points > 21 > self.player.points:
            win = self.player.current_bet
            print(f"You won! Congratulations, {self.player.name.capitalize()}!")
            print(f"You won {win}!")
            self.player.bankroll += win
            self.bankrolls_in_the_end.append(self.player.bankroll)
        elif 21 >= dealer.points > self.player.points or self.player.points > 21:
